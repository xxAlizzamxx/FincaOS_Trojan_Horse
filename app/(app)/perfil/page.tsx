...
export default function PerfilPage() {
  ...
  async function saveNombre() {
    ...
    setSavingName(true);
    await updateDoc(doc(db, 'comunidades', perfil.comunidad_id), { nombre: comunidadNombre.trim() });
  ...
  /* ── Abandonar comunidad ── */
  async function abandonarComunidad() {
    if (!perfil?.id || !perfil?.comunidad_id) return;
    setAbandonando(true);
    try {
      await updateDoc(doc(db, 'perfiles', perfil.id), {
        comunidad_id:  null,
        rol:           'vecino',   // pierde rol especial al salir
        numero_piso:   null,
        torre:         null,
        piso:          null,
        puerta:        null,
      ...
      // Hard reload para limpiar el contexto de useAuth
      window.location.href = '/onboarding';
    } catch (err: any) {
      console.error('[Perfil] Error al abandonar comunidad:', err);
      toast.error('No se pudo abandonar la comunidad');
  ...
  const tieneComunidad = !!perfil?.comunidad_id;
  ...
  return (
    <div className="px-4 py-5 space-y-5 pb-28">
      ...
      <Card className="border-0 shadow-sm overflow-hidden">
        <div className="bg-gradient-to-r from-finca-coral to-finca-salmon px-4 py-5 flex items-center gap-4">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-white font-bold text-xl truncate">{perfil?.nombre_completo || 'Sin nombre'}</p>
            <p className="text-white/80 text-xs truncate mt-0.5">{user?.email}</p>
          </div>
          <Badge className="text-[10px] border shrink-0 bg-white/20 text-white border-white/40">
        ...
        <CardContent className="px-4 pb-4 pt-3">
          <div className="mt-4 grid grid-cols-3 gap-2">
            {[
              { val: torre,       label: 'Torre'      },
              { val: piso,        label: 'Piso'       },
              { val: apartamento, label: 'Apartamento'},
            ].map(({ val, label }) => (
              <div key={label} className="bg-muted/50 rounded-xl p-3 text-center">
                <p className="font-semibold text-finca-dark text-sm">{val || '—'}</p>
      ...
      {/* ── Tarjeta comunidad ── */}
      {tieneComunidad && (perfil?.comunidad as any)?.nombre && (
          ...
          <CardContent className="p-4 space-y-3">
            ...
            {editingName ? (
              <div className="flex gap-2">
                <Input value={comunidadNombre} onChange={(e) => setComunidadNombre(e.target.value)} className="h-9 text-sm" autoFocus />
            ...
            <Button variant="outline" size="sm" className="border-finca-coral text-finca-coral hover:bg-finca-coral hover:text-white w-full" onClick={compartirLink}>
              ...
              <AlertDialogTrigger asChild>
                ...
                >
                  {abandonando
                    ...
                    : <DoorOpen className="w-3.5 h-3.5 mr-2" />}
                  Abandonar comunidad
                </Button>
              </AlertDialogTrigger>
              <AlertDialogContent>
                <AlertDialogHeader>
                  <AlertDialogTitle>¿Abandonar la comunidad?</AlertDialogTitle>
                  <AlertDialogDescription>
                    Dejarás de tener acceso a{' '}
                    <span className="font-semibold text-finca-dark">
                      {(perfil?.comunidad as any)?.nombre}
                    </span>
                    . Podrás unirte a otra comunidad cuando quieras.
                    {esAdmin && (
                      <span className="block mt-2 text-orange-600 font-medium">
                        ⚠️ Perderás tu rol de {rolLabel[perfil?.rol || 'vecino']} al salir.
                      </span>
                    )}
                  </AlertDialogDescription>
                </AlertDialogHeader>
            ...
            <div key={item.label}>
              ...
              >
                <div className="w-9 h-9 rounded-xl bg-finca-peach/40 flex items-center justify-center shrink-0">
                  <item.icon className="w-4 h-4 text-finca-coral" />
                </div>
                <div className="flex-1 min-w-0">
      ...
      <Button variant="outline" className="w-full border-red-200 text-red-500 hover:bg-red-50" onClick={handleSignOut} disabled={loggingOut}>
