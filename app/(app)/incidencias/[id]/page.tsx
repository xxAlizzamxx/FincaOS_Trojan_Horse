      ...
      <div className="px-4 py-4 space-y-4">
        ...
        <div className="space-y-3">
          ...
          {(() => {
            ...
            // Barra: progreso hacia el quórum.
            // Si tenemos porcentaje guardado por el servicio úsalo directamente
            // (porcentaje = count/totalVecinos*100), mapeado a: barra = porcentaje/umbral*100.
            // Fallback para incidencias antiguas sin porcentaje: estimación visual.
            const porcentajeReal = q?.porcentaje as number | undefined;
            const barPct = alcanzado
              ? 100
              : porcentajeReal != null
                ? Math.min(99, Math.round((porcentajeReal / umbral) * 100))
                : Math.min(80, Math.round((count / Math.max(1, count + 3)) * 80));
            return (
