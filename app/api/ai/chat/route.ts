...
async function createIncidenciaFromChat(
  ...
  // ── 0a. Load creator profile (coeficiente for quorum) ───────────────────
  let coef = 1;
  try {
    const perfilSnap = await db.collection('perfiles').doc(uid).get();
    coef = (perfilSnap.data()?.coeficiente as number) ?? 1;
  } catch {
    // Non-fatal — use default coef=1
  }
  ...
  try {
    const ref = await db.collection('incidencias').add({
      ...
      // ────────────────────────────────────────────────
      // ── Quórum (igual que incidencias manuales) ───────
      quorum: {
        tipo:            'simple',
        umbral:          30,
        afectados_count: 1,   // el creador se cuenta desde el inicio
        peso_afectados:  coef,
        alcanzado:       false,
      },
      // ─────────────────────────────────────────────────
    ...
    // Añadir creador como afectado (fire-and-forget — igual que incidencias manuales)
    db.collection('incidencias').doc(incidenciaId)
      .collection('afectados').doc(uid)
      .set({ coeficiente: coef, added_at: now, es_autor: true })
      .catch((e: unknown) => log.error('ai_chat_afectados_subcol_failed', e));
  } catch (err) {
