...
class SoundManager {
  ...
  /* ── Precarga ──
   * Usa fetch HEAD para verificar que el archivo existe antes de crear
   * el elemento Audio. Evita errores 404 ruidosos en la consola.
   */
  preload(...events: SoundEvent[]) {
    ...
    events.forEach(async (event) => {
      ...
      /* Verificar existencia del archivo silenciosamente */
      try {
        const res = await fetch(config.src, { method: 'HEAD' });
        if (!res.ok) {
          log(`Preload skip (no existe): ${config.src} → se usará tono sintetizado`);
          return;
        }
      } catch {
        log(`Preload skip (error red): ${config.src}`);
        return;
      }
